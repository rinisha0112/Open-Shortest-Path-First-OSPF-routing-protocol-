...
void* Recv_LSPF()
{
	...
	{
		pthread_mutex_lock(&lock);
		sendto(sock, lspf_msg, sizeof(lspf_msg),0, (struct sockaddr *) &(peer_list[0]), sizeof(struct sockaddr_in));
		pthread_mutex_unlock(&lock);
		pthread_create(&recv_forward, NULL, Calculate_Route(), NULL);
		pthread_detach(recv_forward);
	}
	else if(strcmp(lspf_msg,"END")==0)
	{
...
void dijkstra(int G[MAX][MAX], int n, int startnode)
{
	...
	FILE *file_ptr=fopen("Routing_Table_B.txt","w");
	...
	printf("Routing_Table_B.txt Successfully created!\n");
