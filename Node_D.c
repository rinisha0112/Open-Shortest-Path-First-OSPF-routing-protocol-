...
void parsed_args(int argc, char **argv)
{
	...
	else
	{
		...
		Neighbour[0].Source='D'; //Source
		Neighbour[0].Cost=3; //Cost
		Neighbour[0].Through='B'; //Through

		
		Neighbour[1].Destination='C';//Destination
		Neighbour[1].Source='D'; //Source
		Neighbour[1].Cost=2; //Cost
...
void* Recv_LSPF()
{
	...
	else if(strcmp(lspf_msg,"ENDENDEND")==0)
	{
...
void dijkstra(int G[MAX][MAX], int n, int startnode)
{
	...
	FILE *file_ptr=fopen("Routing_Table_D.txt","w");
	...
	printf("Routing_Table_D.txt Successfully created!\n");
